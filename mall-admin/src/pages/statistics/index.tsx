...
const Statistics: React.FC = () => {
  ...
  const fetchSalesData = async () => {
    try {
      const response = await fetch(`/api/admin/statistics/sales?${new URLSearchParams({
  ...
  const fetchOrderData = async () => {
    try {
      const response = await fetch(`/api/admin/statistics/orders?${new URLSearchParams({
        startTime: dateRange[0].format('YYYY-MM-DD'),
        endTime: dateRange[1].format('YYYY-MM-DD'),
        timeUnit,
      })}`, {
  ...
  const fetchProductRanking = async () => {
    try {
      const response = await fetch(`/api/admin/statistics/products/ranking?${new URLSearchParams({
        startTime: dateRange[0].format('YYYY-MM-DD'),
        endTime: dateRange[1].format('YYYY-MM-DD'),
      })}`, {
        headers: {
  ...
  const fetchUserRanking = async () => {
    try {
      const response = await fetch(`/api/admin/statistics/users/ranking?${new URLSearchParams({
        startTime: dateRange[0].format('YYYY-MM-DD'),
        endTime: dateRange[1].format('YYYY-MM-DD'),
      })}`, {
        headers: {
